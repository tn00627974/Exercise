很好，這一題我直接幫你設計一個**「真的給新手練功用」**的專案。  
你照著做一遍，**DI / interface / NUnit 會一次全部打通**。

我會給你：

1️⃣ 練習專案的「故事背景」  
2️⃣ 專案結構  
3️⃣ 完整程式碼（可直接建立）  
4️⃣ 練習步驟（你照順序做）

不會用 ASP.NET、不用容器，**先用最純的 DI 思維**。

---

# 一、練習專案主題（簡單又真實）

## 🎯 主題：**訂單通知系統**

需求只有一句話：

> 下訂單後，系統要「發送通知」

通知方式未來可能有：

- Email    
- Line    
- 簡訊    

👉 **這非常適合練 interface + DI + NUnit**

---

# 二、專案結構（照這個建）

建立一個 **Solution**

```
OrderNotificationDemo
│
├─ OrderNotification.Core
│   ├─ INotificationSender.cs
│   ├─ Order.cs
│   ├─ NotificationResult.cs
│   ├─ EmailNotificationSender.cs
│   └─ OrderService.cs
│
└─ OrderNotification.Tests
    ├─ FakeNotificationSender.cs
    └─ OrderServiceTests.cs
```

---

# 三、正式程式碼（Core）

## 1️⃣ INotificationSender.cs（介面）

```csharp
public interface INotificationSender
{
    NotificationResult Send(Order order);
}
```

---

## 2️⃣ Order.cs（DTO）

```csharp
public class Order
{
    public string OrderNo { get; set; }
    public decimal Amount { get; set; }
}
```

---

## 3️⃣ NotificationResult.cs

```csharp
public class NotificationResult
{
    public bool Success { get; set; }
    public string Message { get; set; }
}
```

---

## 4️⃣ EmailNotificationSender.cs（真實實作）

```csharp
public class EmailNotificationSender : INotificationSender
{
    public NotificationResult Send(Order order)
    {
        // 假裝在寄 Email
        return new NotificationResult
        {
            Success = true,
            Message = $"Email 已寄出，訂單：{order.OrderNo}"
        };
    }
}
```

---

## 5️⃣ OrderService.cs（⭐ 重點）

```csharp
public class OrderService
{
    private readonly INotificationSender _sender;

    // 👉 DI：從外面注入
    public OrderService(INotificationSender sender)
    {
        _sender = sender;
    }

    public NotificationResult PlaceOrder(Order order)
    {
        if (order.Amount <= 0)
        {
            return new NotificationResult
            {
                Success = false,
                Message = "訂單金額錯誤"
            };
        }

        return _sender.Send(order);
    }
}
```

📌 **這個 class 是你測試的核心**

---

# 四、測試程式碼（NUnit）

## 6️⃣ FakeNotificationSender.cs（假實作）

```csharp
public class FakeNotificationSender : INotificationSender
{
    public NotificationResult Send(Order order)
    {
        return new NotificationResult
        {
            Success = true,
            Message = "Fake 通知成功"
        };
    }
}
```

---

## 7️⃣ OrderServiceTests.cs

```csharp
using NUnit.Framework;

[TestFixture]
public class OrderServiceTests
{
    [Test]
    public void PlaceOrder_ValidAmount_ShouldSendNotification()
    {
        // Arrange
        var sender = new FakeNotificationSender();
        var service = new OrderService(sender);

        var order = new Order
        {
            OrderNo = "ORD001",
            Amount = 1000
        };

        // Act
        var result = service.PlaceOrder(order);

        // Assert
        Assert.IsTrue(result.Success);
        Assert.AreEqual("Fake 通知成功", result.Message);
    }

    [Test]
    public void PlaceOrder_InvalidAmount_ShouldFail()
    {
        // Arrange
        var sender = new FakeNotificationSender();
        var service = new OrderService(sender);

        var order = new Order
        {
            OrderNo = "ORD002",
            Amount = 0
        };

        // Act
        var result = service.PlaceOrder(order);

        // Assert
        Assert.IsFalse(result.Success);
        Assert.AreEqual("訂單金額錯誤", result.Message);
    }
}
```

---

# 五、你現在「實際學到」的是什麼？

### ✅ Interface

- `INotificationSender` 是合約
    
- Email / Fake 都可以替換
    

### ✅ DI（建構式注入）

- `OrderService` **不自己 new**
    
- 外面決定要用誰
    

### ✅ NUnit

- 不連外    
- 不靠 UI    
- 測的是「行為」
    

---

# 六、建議你照這個順序練（很重要）

### 🔁 第 1 關（現在）

✔ 跑過測試  
✔ 確定全部綠燈

---

### 🔁 第 2 關（你自己改）

- 新增 `LineNotificationSender`
    
- 不改 `OrderService`
    
- 測試仍然要過
    

---

### 🔁 第 3 關（進階）

- 讓 `FakeNotificationSender` 可以「指定成功或失敗」    
- 測失敗情境
    
---

### 🔁 第 4 關（進階 - 依賴注入深化）

- 新增 `IOrderValidator` 介面
    
- `OrderService` 同時注入「驗證器」和「通知發送器」
    
- 驗證失敗時，不呼叫 `Send()`
    
- 用 `Substitute` Mock 驗證 `Send()` 呼叫次數

**重點學習：**
- ✅ 多個介面依賴注入
- ✅ 關注點分離（Validation vs Notification）
- ✅ Mock 驗證方法呼叫（`Received()` / `DidNotReceive()`）

---

### 🔁 第 5 關（進階 - 真實場景：Composite Pattern）

#### 📋 新需求

訂單下單後，有時候需要「同時發送多個通知」

例如：同時寄 Email **和** 發送 Line

#### 🎯 場景案例

```csharp
// ❓ 現在怎麼同時用這兩個？
// OrderService 只能注入一個 INotificationSender...

var emailSender = new EmailNotificationSender();
var lineSender = new LineNotificationSender();

// 需要同時發送 Email 和 Line
```

#### 💡 解決方案：Composite Pattern

**建立複合通知發送器：**

```csharp
// OrderNotificationSystem\CompositeNotificationSender.cs
using System;
using System.Collections.Generic;
using System.Linq;

namespace OrderNotificationSystem
{
    /// <summary>
    /// 複合通知發送器：同時使用多個發送器
    /// </summary>
    public class CompositeNotificationSender : INotificationSender
    {
        private readonly IEnumerable<INotificationSender> _senders;

        public CompositeNotificationSender(params INotificationSender[] senders)
        {
            _senders = senders;
        }

        public NotificationResult Send(Order order)
        {
            var results = new List<NotificationResult>();
            var failedMessages = new List<string>();

            // 逐一執行所有發送器
            foreach (var sender in _senders)
            {
                try
                {
                    var result = sender.Send(order);
                    results.Add(result);

                    if (!result.Success)
                    {
                        failedMessages.Add(result.Message);
                    }
                }
                catch (Exception ex)
                {
                    failedMessages.Add($"發送失敗: {ex.Message}");
                }
            }

            // 決定整體結果
            if (failedMessages.Any())
            {
                return new NotificationResult
                {
                    Success = false,
                    Message = $"部分通知失敗: {string.Join("; ", failedMessages)}"
                };
            }

            return new NotificationResult
            {
                Success = true,
                Message = $"所有通知已發送 ({results.Count} 個)"
            };
        }
    }
}
```

#### 🧪 測試案例

```csharp
// OrderNotificationSystemTests\CompositeNotificationSenderTests.cs
using NSubstitute;
using OrderNotificationSystem;
using OrderNotificationSystem.Models;

namespace OrderNotificationSystemTests
{
    public class CompositeNotificationSenderTests
    {
        [Test]
        public void Send_WithMultipleSenders_AllSuccess()
        {
            // Arrange
            var order = new Order { OrderNo = "ORD001", Amount = 1000 };
            
            var emailSender = Substitute.For<INotificationSender>();
            var lineSender = Substitute.For<INotificationSender>();
            
            emailSender.Send(Arg.Any<Order>()).Returns(new NotificationResult 
            { 
                Success = true, 
                Message = "Email sent" 
            });
            
            lineSender.Send(Arg.Any<Order>()).Returns(new NotificationResult 
            { 
                Success = true, 
                Message = "Line sent" 
            });

            var composite = new CompositeNotificationSender(emailSender, lineSender);

            // Act
            var result = composite.Send(order);

            // Assert
            Assert.That(result.Success, Is.True);
            Assert.That(result.Message, Contains.Substring("2"));
            
            // 👉 驗證兩個發送器都被呼叫
            emailSender.Received(1).Send(order);
            lineSender.Received(1).Send(order);
        }

        [Test]
        public void Send_WhenOneSenderFails_ReturnsFailure()
        {
            // Arrange
            var order = new Order { OrderNo = "ORD002", Amount = 500 };
            
            var emailSender = Substitute.For<INotificationSender>();
            var lineSender = Substitute.For<INotificationSender>();
            
            emailSender.Send(Arg.Any<Order>()).Returns(new NotificationResult 
            { 
                Success = true, 
                Message = "Email sent" 
            });
            
            lineSender.Send(Arg.Any<Order>()).Returns(new NotificationResult 
            { 
                Success = false, 
                Message = "Line service down" 
            });

            var composite = new CompositeNotificationSender(emailSender, lineSender);

            // Act
            var result = composite.Send(order);

            // Assert
            Assert.That(result.Success, Is.False);
            Assert.That(result.Message, Contains.Substring("Line service down"));
        }

        [Test]
        public void Send_WithRealSenders_EmailAndLine()
        {
            // Arrange - 整合測試：使用真實實作
            var order = new Order { OrderNo = "ORD003", Amount = 1500 };
            
            INotificationSender emailSender = new EmailNotificationSender();
            INotificationSender lineSender = new LineNotificationSender();
            
            var composite = new CompositeNotificationSender(emailSender, lineSender);

            // Act
            var result = composite.Send(order);

            // Assert
            Assert.That(result.Success, Is.True);
            Assert.That(result.Message, Contains.Substring("所有通知已發送"));
        }
    }
}
```

#### 🎯 用法示例

**原來（只能一個）：**
```csharp
INotificationSender sender = new EmailNotificationSender();
var orderService = new OrderService(_validator, sender);
```

**現在（可以多個）：**
```csharp
// 同時發送 Email 和 Line
INotificationSender composite = new CompositeNotificationSender(
    new EmailNotificationSender(),
    new LineNotificationSender()
);
var orderService = new OrderService(_validator, composite);
```

#### 📚 你學到的新概念

### ✅ **Composite Pattern（組合模式）**
- `CompositeNotificationSender` 本身也是 `INotificationSender`
- 可以包含多個 `INotificationSender`
- 對外表現跟單一發送器一樣（透明性）
- 可以無限組合嵌套

### ✅ **Exception Handling（異常處理）**
- 部分失敗不會導致全部失敗
- 記錄每個失敗的原因
- 使用 try-catch 捕捉異常

### ✅ **Collections（集合）**
- 使用 `IEnumerable<T>` 存放多個依賴
- 使用 `params` 簡化建構子
- 使用 `Linq` 查詢集合

**重點學習：**
- ✅ 設計模式：Composite Pattern
- ✅ 異常處理和容錯
- ✅ 集合操作和迴圈

---

# 七、一句新手一定要記住的話

> **「我能用 Fake 測試，代表我架構是對的」**

---

# 八、進階企業用法（選修）⭐⭐⭐

當你完成第關卡後，可以學習企業標準的做法。

## 企業標準的測試組織方式

### 📁 推薦的專案結構

```
OrderNotificationSystem/
├─ Core/
│   ├─ INotificationSender.cs
│   ├─ IOrderValidator.cs
│   ├─ Order.cs
│   ├─ NotificationResult.cs
│   ├─ EmailNotificationSender.cs
│   ├─ LineNotificationSender.cs
│   ├─ DefaultOrderValidator.cs
│   └─ OrderService.cs
│
└─ Tests/
    ├─ Unit/                           # 👈 單元測試資料夾
    │   ├─ OrderServiceTests.cs
    │   ├─ DefaultOrderValidatorTests.cs
    │   └─ FakeNotificationSenderTests.cs
    │
    └─ Integration/                    # 👈 整合測試資料夾
        └─ OrderServiceIntegrationTests.cs
```

### 📊 為什麼要分開？

| 項目 | 單元測試 | 整合測試 |
|------|---------|---------|
| **檔案位置** | `Tests/Unit/` | `Tests/Integration/` |
| **使用 Mock** | ✅ Substitute | ❌ 不用 |
| **使用真實類** | ❌ 隔離 | ✅ 真實實作 |
| **執行速度** | ⚡ 快（秒級） | 🐌 慢（分鐘級） |
| **執行時機** | 每次存檔 | 定期/提交前 |
| **覆蓋範圍** | 單一類別/方法 | 多個類別協作 |

### ✅ 單元測試範例：OrderServiceTests.cs

```csharp
using NSubstitute;
using OrderNotificationSystem.Models;
using OrderNotificationSystem;

namespace OrderNotificationSystemTests.Unit
{
    public class OrderServiceTests
    {
        private INotificationSender _mockSender;
        private IOrderValidator _mockValidator;

        [SetUp]
        public void Setup()
        {
            _mockSender = Substitute.For<INotificationSender>();
            _mockValidator = Substitute.For<IOrderValidator>();
        }

        [Test]
        public void PlaceOrder_When_ValidatorPasses_ShouldCallSender()
        {
            // Arrange - 用 Mock 隔離依賴
            var order = new Order { OrderNo = "order001", Amount = 100 };
            _mockValidator.Validate(Arg.Any<Order>()).Returns(new NotificationResult 
            { 
                Success = true, 
                Message = "驗證通過" 
            });

            var orderService = new OrderService(_mockValidator, _mockSender);

            // Act
            var result = orderService.PlaceOrder(order);

            // Assert
            Assert.That(result.Success, Is.True);
            
            // 👉 驗證行為：Send 被呼叫 1 次
            _mockSender.Received(1).Send(order);
        }

        [Test]
        public void PlaceOrder_When_ValidatorFails_ShouldNotCallSender()
        {
            // Arrange - 驗證失敗的情況
            var order = new Order { OrderNo = "order001", Amount = -100 };
            _mockValidator.Validate(Arg.Any<Order>()).Returns(new NotificationResult 
            { 
                Success = false, 
                Message = "訂單錯誤" 
            });

            var orderService = new OrderService(_mockValidator, _mockSender);

            // Act
            var result = orderService.PlaceOrder(order);

            // Assert
            Assert.That(result.Success, Is.False);
            
            // 👉 驗證行為：Send 沒有被呼叫
            _mockSender.DidNotReceive().Send(Arg.Any<Order>());
        }
    }
}
```

### ✅ 整合測試範例：OrderServiceIntegrationTests.cs

```csharp
using OrderNotificationSystem.Models;
using OrderNotificationSystem;

namespace OrderNotificationSystemTests.Integration
{
    public class OrderServiceIntegrationTests
    {
        [Test]
        public void PlaceOrder_WithRealValidator_AndRealEmailSender_CompleteFlow()
        {
            // Arrange - 使用真實實作，不用 Mock
            var order = new Order { OrderNo = "ORD-2024-001", Amount = 1000 };
            IOrderValidator validator = new DefaultOrderValidator();
            INotificationSender sender = new EmailNotificationSender();
            var orderService = new OrderService(validator, sender);

            // Act - 執行完整流程
            var result = orderService.PlaceOrder(order);

            // Assert - 驗證端對端流程
            Assert.That(result.Success, Is.True);
            Assert.That(result.Message, Is.EqualTo($"訂單金額為{order.Amount}元"));
        }

        [Test]
        public void PlaceOrder_WithRealValidator_AndRealLineSender_CompleteFlow()
        {
            // Arrange - 使用真實實作測試不同發送者
            var order = new Order { OrderNo = "ORD-2024-002", Amount = 500 };
            IOrderValidator validator = new DefaultOrderValidator();
            INotificationSender sender = new LineNotificationSender();
            var orderService = new OrderService(validator, sender);

            // Act
            var result = orderService.PlaceOrder(order);

            // Assert
            Assert.That(result.Success, Is.True);
            Assert.That(result.Message, Is.EqualTo($"訂單金額為{order.Amount}元"));
        }

        [Test]
        public void PlaceOrder_WithInvalidAmount_FailsValidation()
        {
            // Arrange - 整合測試驗證流程：驗證失敗 → 不發送
            var order = new Order { OrderNo = "ORD-2024-003", Amount = -100 };
            IOrderValidator validator = new DefaultOrderValidator();
            INotificationSender sender = new EmailNotificationSender();
            var orderService = new OrderService(validator, sender);

            // Act
            var result = orderService.PlaceOrder(order);

            // Assert - 驗證整個流程的行為
            Assert.That(result.Success, Is.False);
            Assert.That(result.Message, Is.EqualTo("訂單錯誤"));
        }
    }
}
```

### 🎯 測試金字塔

```
        📊 E2E 測試
       /            \
      少量         (1個)
     /                  \
    整合測試           (少量)
   /                      \
  單元測試              (大量 ⭐)
 /____________________________\

比例：
- 單元測試 80%  → 快速反饋，每次存檔執行
- 整合測試 15%  → 驗證協作，提交前執行
- E2E 測試 5%   → 完整流程，部署前執行
```

### ✨ 企業做法優勢

✅ **快速開發**
- 只執行單元測試：秒級反饋
- 立即發現邏輯問題

✅ **可靠部署**
- 單元測試 ✓ + 整合測試 ✓ = 高信心
- 清楚知道問題在哪一層

✅ **易於維護**
- 測試職責清晰
- 每個資料夾一個目的

✅ **持續整合**
- CI/CD 可分階段執行
- 快速失敗（fail fast）

### 🚀 如何實施

1. **建立資料夾結構**
   ```
   Tests/
   ├── Unit/
   └── Integration/
   ```

2. **遷移現有測試**
   - Mock 的 → `Unit/`
   - 真實實作 → `Integration/`

3. **設定測試執行**
   ```bash
   # 快速反饋（開發中）
   dotnet test --filter "Category=Unit"
   
   # 完整測試（提交前）
   dotnet test
   ```

4. **在 CI/CD 中使用**
   ```yaml
   # 開發階段：只跑單元測試（快）
   jobs:
     - script: dotnet test --filter "Category=Unit"
   
   # 部署前：跑全部測試（完整）
     - script: dotnet test
   ```

---

如果你願意，下一步我可以直接出「作業題」給你，例如：

- ❓ 哪個地方「不該抽介面」    
- 🧪 請你補一個測試（我幫你改）    
- 💣 我故意寫一個壞版本，讓你修
    

你只要說：  
👉「**給我下一關**」